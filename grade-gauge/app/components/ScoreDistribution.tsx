    ...
    <div className="space-y-1.5">
      {bands.map((band) => (
        <div key={band.label} className="flex items-center gap-3 text-sm">
          <span className="w-24 shrink-0 text-gray-500 dark:text-gray-400">{band.label}</span>
          ...
          <span className="w-6 shrink-0 text-right font-medium text-gray-700 dark:text-gray-300">{band.count}</span>
        </div>
      ))}
    </div>
  );
